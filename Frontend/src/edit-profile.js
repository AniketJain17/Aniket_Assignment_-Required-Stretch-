...
function fetchUserInfo(userId, one) {
  ...
  fetch(
    `https://bytive-assignment-stretch-frn4.vercel.app/api/user/userprofile/${userId}`,
    {
      headers: headers,
    }
  )
...
function renderuser(user, one) {
  ...
  <div class="row">
    ...
    <div class="col-md-6">
      <img style="border-radius:9px;border:1px solid black" src="${
        user.gravatar
    ...
    <div class="col-md-6">
      <div class="card-body">
        ...
        <p class="card-text"><strong>GitHub:</strong> <a href="${
          user.githubURL
        }" target="_blank">${user.githubURL}</a></p>
        <p class="card-text"><strong>Twitter:</strong> <a href="${
          user.twitterURL
        }" target="_blank">${user.twitterURL}</a></p>
        <p class="card-text"><strong>Website:</strong> <a href="${
          user.websiteURL
        }" target="_blank">${user.websiteURL}</a></p>
        <p class="card-text"><strong>LinkedIn:</strong> <a href="${
          user.linkedinURL
        }" target="_blank">${user.linkedinURL}</a></p>
        <p class="card-text"><strong>Tech Stack:</strong> ${user.techStack.join(
          ", "
        )}</p>
        <p class="card-text"><strong>Field of Interest:</strong> ${user.fieldOfInterest.join(
          ", "
        )}</p>
        <p class="card-text"><strong>Seeking:</strong> ${user.seeking.join(
          ", "
        )}</p>
      </div>
...
`);
  ...
  <div class="col-md-6">
    ...
    <button class="btn btn-danger card-text" onclick="deleteProfile('${
      user._id
...
<hr>
  ...
  <div class="col-md-6">
    ...
    <div class="mb-4">
      ...
      <input type="text" class="form-control" id="location" required value="${
        user.location
      }">
    </div>
  </div>
  ...
  <div class="col-md-6">
    <div class="mb-4">
      ...
      <input type="url" class="form-control" id="gravatar" required value="${
        user.gravatar
      }">
    </div>
    ...
    <div class="mb-4">
      ...
      <textarea class="form-control" id="bio" rows="3" required>${
        user.bio
      }</textarea>
    </div>
    ...
    <div class="mb-4">
      ...
      <input type="url" class="form-control" id="githubURL" required value="${
        user.githubURL
      }">
    </div>
    ...
    <div class="mb-4">
      ...
      <input type="url" class="form-control" id="linkedinURL" required value="${
        user.linkedinURL
      }">
    </div>
    ...
    <div class="mb-4">
      ...
      <input type="url" class="form-control" id="twitterURL" required value="${
        user.twitterURL
      }">
    </div>
    ...
    <div class="mb-4">
      ...
      <input type="url" class="form-control" id="websiteURL" required value="${
        user.websiteURL
      }">
    </div>
  </div>
...
$(document).ready(function () {
  $(document).on("submit", "#updateForm", function (event) {
    ...
    $.ajax({
      url: `https://bytive-assignment-stretch-frn4.vercel.app/api/user/editprofile/${loggedInId}`,
      method: "PUT",
      headers: {
